...
def fit_in_best_fit_interval(f: np.ndarray[float], times: np.ndarray[float], start: int = None, end: int = None):
    ...
    # Use slope on the best sub-interval (not full interval) for consistency with overhaul
    if int_len > 1:
        intervals = np.linspace(start, end, int_len, dtype=int)
        best_fit_interval = min(best_fit_interval, len(intervals) - 2)
        a, b = int(intervals[best_fit_interval]), int(intervals[best_fit_interval + 1])
        slope, intercept, r_value, p_value, std_err = linregress(times[a:b], f[a:b])
        fit_dict = {'fit_err': std_err, 'slope_err': min_mae, 'interval': [a, b]}
    else:

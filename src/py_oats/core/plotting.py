...
def plot_correlation_pair(analyzer : OnsagerTransportAnalyzer, i : Union[str, int, Element], j : Union[str, int, Element], log=False, return_msd=False):
    ...
    plot_msd(analyzer.msds[(i, j)], labels=[analyzer.species[i], analyzer.species[j]], log=log)
    if return_msd:
        return analyzer.msds[(i, j)]
...
def plot_correlation_pairwise(analyzer: OnsagerTransportAnalyzer, size=10):
    ...
    for i in range(len(analyzer.species)):
        for j in range(i, len(analyzer.species)):
            if i != j:
                plot_msd(analyzer, analyzer.msds[(i, j)], labels=[analyzer.species[i], analyzer.species[j]], size=size)
...
def plot_all_correlations(analyzer: OnsagerTransportAnalyzer, size=10):
    ...
    for i in range(len(analyzer.species)):
        for j in range(i, len(analyzer.species)):
            if i != j:
                plt.plot(analyzer.times, analyzer.msds[(i, j)][4], label=analyzer.species[i] + '-' + analyzer.species[j])
                plt.plot(analyzer.times, analyzer.msds[(i, j)][1], label=analyzer.species[i] + '-' + analyzer.species[i] + '_self')
                plt.plot(analyzer.times, analyzer.msds[(i, j)][0], label=analyzer.species[i] + '-' + analyzer.species[i] + '_net')
    plt.xlabel('Time (fs)')
